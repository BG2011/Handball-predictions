...
class HandballPredictor(BaseHandballPredictor):
    ...
    def train_models(self, X_train: pd.DataFrame, y_result: pd.Series, y_home_goals: pd.Series, y_away_goals: pd.Series):
        ...
        # Użyj parametrów z konfiguracji i zwiększ n_estimators
        xgb_params = config.ml.xgb_params.copy()
        xgb_params['n_estimators'] = 200  # Zwiększona wartość dla lepszej jakości
    ...
    def save_predictions_with_details(self, predictions: List[Dict], filename: str = None):
        ...
        # Dodatkowe metadane dla tego modelu
        additional_metadata = {
        ...
        # Użyj metody z klasy bazowej z dodatkowymi metadanymi
        return super().save_predictions(predictions, filename)
